...
export default function PerformanceKPIs() {
  ...
  const sectionData = kpiData.filter(d => d.section.startsWith("9."));

  useEffect(() => {
    if (!isLoading && sectionData.length === 0) {
      apiRequest("POST", "/api/leadership-kpi-data/seed-section", { prefix: "9" }).then(() => {
  ...
  const section91 = sectionData.filter(d => d.section === "9.1");
  const section92 = sectionData.filter(d => d.section === "9.2");
  const section93 = sectionData.filter(d => d.section === "9.3");
  ...
  const handleExportImage = async () => {
    ...
    link.download = `performance-kpis-${new Date().toISOString().split("T")[0]}.png`;
  ...
  const handleExportPDF = async () => {
    ...
    pdf.save(`performance-kpis-${new Date().toISOString().split("T")[0]}.pdf`);
  };
        ...
        <CardContent className="space-y-6">
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b bg-muted/50">
                  <th className="text-start py-3 px-3 font-medium w-[35%]">{t('performanceKpis.indicator')}</th>
                  <th className="text-start py-3 px-3 font-medium w-[30%]">{t('performanceKpis.measurementMethod')}</th>
                  <th className="text-start py-3 px-3 font-medium w-[12%]">{t('performanceKpis.targetLabel')}</th>
                  <th className="text-start py-3 px-3 font-medium w-[12%]">{t('performanceKpis.actualValue')}</th>
            ...
            <div className="h-72 mt-4">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                  ...
                  <Tooltip
                    formatter={(value: number, name: string) => [value, name === "actual" ? t('performanceKpis.actualValue') : t('performanceKpis.targetLabel')]}
                    labelFormatter={(label) => { const entry = chartData.find(d => d.name === label); return entry?.fullName || label; }}
                  />
                  <Bar dataKey="target" fill="#cbd5e1" name={t('performanceKpis.targetLabel')} barSize={28} radius={[4, 4, 0, 0]}>
                    <LabelList dataKey="target" position="top" fontSize={10} fill="#64748b" />
                  </Bar>
                  <Bar dataKey="actual" name={t('performanceKpis.actualValue')} barSize={28} radius={[4, 4, 0, 0]}>
          ...
          {!hasActualValues && data.length > 0 && (
            <div className="text-center py-6 text-muted-foreground text-sm" data-testid={`text-no-chart-${sectionId}`}>
              {t('performanceKpis.fillActualValues')}
            </div>
          )}
                    ...
                    <CardContent className="p-4">
                      ...
                      <div className="space-y-1.5">
                        <div className="flex justify-between text-xs">
                          <span className="text-muted-foreground">{t('performanceKpis.targetLabel')}:</span>
                          <span className="font-medium">{item.target || "—"}</span>
                        </div>
                        <div className="flex justify-between text-xs">
                          <span className="text-muted-foreground">{t('performanceKpis.actualValue')}:</span>
  ...
  if (isLoading) {
    return (
      <div className="p-6">
        <PageHeader title={t('performanceKpis.title')} description={t('performanceKpis.description')} clause="9" />
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={handleExportImage} data-testid="button-export-image">
            ...
            {t('performanceKpis.exportImage')}
          </Button>
          <Button variant="outline" size="sm" onClick={handleExportPDF} data-testid="button-export-pdf">
            <FileDown className="h-4 w-4 ltr:mr-2 rtl:ml-2" />
            {t('performanceKpis.exportPDF')}
          </Button>
        </div>
      </div>

      <div ref={kpiRef}>
        {renderKpiSection("9.1", t('performanceKpis.section91Title'), section91)}
        {renderKpiSection("9.2", t('performanceKpis.section92Title'), section92)}
        {renderKpiSection("9.3", t('performanceKpis.section93Title'), section93)}
      </div>
    </div>
  );
}
