...
export default function ImprovementKPIs() {
  ...
  const sectionData = kpiData.filter(d => d.section.startsWith("10."));

  useEffect(() => {
    if (!isLoading && sectionData.length === 0) {
      apiRequest("POST", "/api/leadership-kpi-data/seed-section", { prefix: "10" }).then(() => {
  ...
  const section101 = sectionData.filter(d => d.section === "10.1");
  const section102 = sectionData.filter(d => d.section === "10.2");
  const section103 = sectionData.filter(d => d.section === "10.3");
  ...
  const handleExportImage = async () => {
    ...
    link.download = `improvement-kpis-${new Date().toISOString().split("T")[0]}.png`;
  ...
  const handleExportPDF = async () => {
    ...
    pdf.save(`improvement-kpis-${new Date().toISOString().split("T")[0]}.pdf`);
  };
        ...
        <CardContent className="space-y-6">
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b bg-muted/50">
                  <th className="text-start py-3 px-3 font-medium w-[35%]">{t('improvementKpis.indicator')}</th>
                  <th className="text-start py-3 px-3 font-medium w-[30%]">{t('improvementKpis.measurementMethod')}</th>
                  <th className="text-start py-3 px-3 font-medium w-[12%]">{t('improvementKpis.targetLabel')}</th>
                  <th className="text-start py-3 px-3 font-medium w-[12%]">{t('improvementKpis.actualValue')}</th>
            ...
            <div className="h-72 mt-4">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                  ...
                  <Tooltip
                    formatter={(value: number, name: string) => [value, name === "actual" ? t('improvementKpis.actualValue') : t('improvementKpis.targetLabel')]}
                    labelFormatter={(label) => { const entry = chartData.find(d => d.name === label); return entry?.fullName || label; }}
                  />
                  <Bar dataKey="target" fill="#cbd5e1" name={t('improvementKpis.targetLabel')} barSize={28} radius={[4, 4, 0, 0]}>
                    <LabelList dataKey="target" position="top" fontSize={10} fill="#64748b" />
                  </Bar>
                  <Bar dataKey="actual" name={t('improvementKpis.actualValue')} barSize={28} radius={[4, 4, 0, 0]}>
          ...
          {!hasActualValues && data.length > 0 && (
            <div className="text-center py-6 text-muted-foreground text-sm" data-testid={`text-no-chart-${sectionId}`}>
              {t('improvementKpis.fillActualValues')}
            </div>
          )}
                    ...
                    <CardContent className="p-4">
                      ...
                      <div className="space-y-1.5">
                        <div className="flex justify-between text-xs">
                          <span className="text-muted-foreground">{t('improvementKpis.targetLabel')}:</span>
                          <span className="font-medium">{item.target || "—"}</span>
                        </div>
                        <div className="flex justify-between text-xs">
                          <span className="text-muted-foreground">{t('improvementKpis.actualValue')}:</span>
  ...
  if (isLoading) {
    return (
      <div className="p-6">
        <PageHeader title={t('improvementKpis.title')} description={t('improvementKpis.description')} clause="10" />
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={handleExportImage} data-testid="button-export-image">
            ...
            {t('improvementKpis.exportImage')}
          </Button>
          <Button variant="outline" size="sm" onClick={handleExportPDF} data-testid="button-export-pdf">
            <FileDown className="h-4 w-4 ltr:mr-2 rtl:ml-2" />
            {t('improvementKpis.exportPDF')}
          </Button>
        </div>
      </div>

      <div ref={kpiRef}>
        {renderKpiSection("10.1", t('improvementKpis.section101Title'), section101)}
        {renderKpiSection("10.2", t('improvementKpis.section102Title'), section102)}
        {renderKpiSection("10.3", t('improvementKpis.section103Title'), section103)}
      </div>
    </div>
  );
}
