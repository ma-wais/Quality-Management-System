...
export default function OperationKPIs() {
  ...
  const sectionData = kpiData.filter(d => d.section.startsWith("8."));

  useEffect(() => {
    if (!isLoading && sectionData.length === 0) {
      apiRequest("POST", "/api/leadership-kpi-data/seed-section", { prefix: "8" }).then(() => {
  ...
  const section81 = sectionData.filter(d => d.section === "8.1");
  const section82 = sectionData.filter(d => d.section === "8.2");
  const section83 = sectionData.filter(d => d.section === "8.3");
  const section84 = sectionData.filter(d => d.section === "8.4");
  const section85 = sectionData.filter(d => d.section === "8.5");
  const section86 = sectionData.filter(d => d.section === "8.6");
  const section87 = sectionData.filter(d => d.section === "8.7");
  ...
  const handleExportImage = async () => {
    ...
    link.download = `operation-kpis-${new Date().toISOString().split("T")[0]}.png`;
  ...
  const handleExportPDF = async () => {
    ...
    pdf.save(`operation-kpis-${new Date().toISOString().split("T")[0]}.pdf`);
  };
        ...
        <CardContent className="space-y-6">
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b bg-muted/50">
                  <th className="text-start py-3 px-3 font-medium w-[35%]">{t('operationKpis.indicator')}</th>
                  <th className="text-start py-3 px-3 font-medium w-[30%]">{t('operationKpis.measurementMethod')}</th>
                  <th className="text-start py-3 px-3 font-medium w-[12%]">{t('operationKpis.targetLabel')}</th>
                  <th className="text-start py-3 px-3 font-medium w-[12%]">{t('operationKpis.actualValue')}</th>
            ...
            <div className="h-72 mt-4">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                  ...
                  <Tooltip
                    formatter={(value: number, name: string) => [value, name === "actual" ? t('operationKpis.actualValue') : t('operationKpis.targetLabel')]}
                    labelFormatter={(label) => { const entry = chartData.find(d => d.name === label); return entry?.fullName || label; }}
                  />
                  <Bar dataKey="target" fill="#cbd5e1" name={t('operationKpis.targetLabel')} barSize={28} radius={[4, 4, 0, 0]}>
                    <LabelList dataKey="target" position="top" fontSize={10} fill="#64748b" />
                  </Bar>
                  <Bar dataKey="actual" name={t('operationKpis.actualValue')} barSize={28} radius={[4, 4, 0, 0]}>
          ...
          {!hasActualValues && data.length > 0 && (
            <div className="text-center py-6 text-muted-foreground text-sm" data-testid={`text-no-chart-${sectionId}`}>
              {t('operationKpis.fillActualValues')}
            </div>
          )}
                    ...
                    <CardContent className="p-4">
                      ...
                      <div className="space-y-1.5">
                        <div className="flex justify-between text-xs">
                          <span className="text-muted-foreground">{t('operationKpis.targetLabel')}:</span>
                          <span className="font-medium">{item.target || "—"}</span>
                        </div>
                        <div className="flex justify-between text-xs">
                          <span className="text-muted-foreground">{t('operationKpis.actualValue')}:</span>
  ...
  if (isLoading) {
    return (
      <div className="p-6">
        <PageHeader title={t('operationKpis.title')} description={t('operationKpis.description')} clause="8" />
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={handleExportImage} data-testid="button-export-image">
            ...
            {t('operationKpis.exportImage')}
          </Button>
          <Button variant="outline" size="sm" onClick={handleExportPDF} data-testid="button-export-pdf">
            <FileDown className="h-4 w-4 ltr:mr-2 rtl:ml-2" />
            {t('operationKpis.exportPDF')}
          </Button>
        </div>
      </div>

      <div ref={kpiRef}>
        {renderKpiSection("8.1", t('operationKpis.section81Title'), section81)}
        {renderKpiSection("8.2", t('operationKpis.section82Title'), section82)}
        {renderKpiSection("8.3", t('operationKpis.section83Title'), section83)}
        {renderKpiSection("8.4", t('operationKpis.section84Title'), section84)}
        {renderKpiSection("8.5", t('operationKpis.section85Title'), section85)}
        {renderKpiSection("8.6", t('operationKpis.section86Title'), section86)}
        {renderKpiSection("8.7", t('operationKpis.section87Title'), section87)}
      </div>
    </div>
  );
}
