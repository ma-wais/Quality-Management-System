...
export default function ContextKPIs() {
  ...
  const contextData = kpiData.filter(d => d.section.startsWith("4."));

  useEffect(() => {
    if (!isLoading && contextData.length === 0) {
      apiRequest("POST", "/api/leadership-kpi-data/seed-context").then(() => {
        queryClient.invalidateQueries({ queryKey: ["/api/leadership-kpi-data"] });
      }).catch(() => {});
    }
  }, [contextData.length, isLoading]);
  ...
  const section41 = contextData.filter(d => d.section === "4.1");
  const section42 = contextData.filter(d => d.section === "4.2");
  const section43 = contextData.filter(d => d.section === "4.3");
  const section44 = contextData.filter(d => d.section === "4.4");
  ...
  const handleExportImage = async () => {
    ...
    link.download = `context-kpis-${new Date().toISOString().split("T")[0]}.png`;
  ...
  const handleExportPDF = async () => {
    ...
    pdf.save(`context-kpis-${new Date().toISOString().split("T")[0]}.pdf`);
  };
        ...
        <CardContent className="space-y-6">
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b bg-muted/50">
                  <th className="text-start py-3 px-3 font-medium w-[35%]">{t('contextKpis.indicator')}</th>
                  <th className="text-start py-3 px-3 font-medium w-[30%]">{t('contextKpis.measurementMethod')}</th>
                  <th className="text-start py-3 px-3 font-medium w-[12%]">{t('contextKpis.targetLabel')}</th>
                  <th className="text-start py-3 px-3 font-medium w-[12%]">{t('contextKpis.actualValue')}</th>
                ...
                <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                  ...
                  <Tooltip
                    formatter={(value: number, name: string) => [
                      value,
                      name === "actual" ? t('contextKpis.actualValue') : t('contextKpis.targetLabel')
                    ]}
                  ...
                  />
                  <Bar dataKey="target" fill="#cbd5e1" name={t('contextKpis.targetLabel')} barSize={28} radius={[4, 4, 0, 0]}>
                    <LabelList dataKey="target" position="top" fontSize={10} fill="#64748b" />
                  </Bar>
                  <Bar dataKey="actual" name={t('contextKpis.actualValue')} barSize={28} radius={[4, 4, 0, 0]}>
                    {chartData.map((entry, index) => (
                      <Cell key={index} fill={getBarColor(entry.actual, entry.target)} />
                    ))}
          ...
          {!hasActualValues && data.length > 0 && (
            <div className="text-center py-6 text-muted-foreground text-sm" data-testid={`text-no-chart-${sectionId}`}>
              {t('contextKpis.fillActualValues')}
            </div>
          )}
                    ...
                    <CardContent className="p-4">
                      ...
                      <div className="space-y-1.5">
                        <div className="flex justify-between text-xs">
                          <span className="text-muted-foreground">{t('contextKpis.targetLabel')}:</span>
                          <span className="font-medium">{item.target || "—"}</span>
                        </div>
                        <div className="flex justify-between text-xs">
                          <span className="text-muted-foreground">{t('contextKpis.actualValue')}:</span>
                            ...
                            <div className="h-2 bg-muted rounded-full overflow-hidden">
                              <div
                                ...
                                style={{
                                  ...
                                  backgroundColor: getBarColor(actual, target),
                                }}
                              />
                            </div>
  ...
  if (isLoading) {
    return (
      <div className="p-6">
        <PageHeader title={t('contextKpis.title')} description={t('contextKpis.description')} clause="4" />
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={handleExportImage} data-testid="button-export-image">
            ...
            {t('contextKpis.exportImage')}
          </Button>
          <Button variant="outline" size="sm" onClick={handleExportPDF} data-testid="button-export-pdf">
            <FileDown className="h-4 w-4 ltr:mr-2 rtl:ml-2" />
            {t('contextKpis.exportPDF')}
          </Button>
        </div>
      </div>

      <div ref={kpiRef}>
        {renderKpiSection("4.1", t('contextKpis.section41Title'), section41)}
        {renderKpiSection("4.2", t('contextKpis.section42Title'), section42)}
        {renderKpiSection("4.3", t('contextKpis.section43Title'), section43)}
        {renderKpiSection("4.4", t('contextKpis.section44Title'), section44)}
      </div>
    </div>
  );
}
...
function KpiRow({ item, onUpdate, onDelete }: {
  ...
  const handleSave = () => {
    onUpdate({
      id: item.id,
      data: { indicator, measurementMethod: method, target, actualValue: actual },
    });
    setDirty(false);
  };
  ...
  return (
    <tr className="border-b last:border-0 hover:bg-muted/30" data-testid={`row-kpi-${item.id}`}>
      <td className="py-2 px-3">
        <Input
          value={indicator}
          onChange={handleChange(setIndicator)}
          className="h-9 text-sm"
          data-testid={`input-indicator-${item.id}`}
        />
      </td>
      <td className="py-2 px-3">
        <Input
          value={method}
          onChange={handleChange(setMethod)}
          className="h-9 text-sm"
          data-testid={`input-method-${item.id}`}
        />
      </td>
      <td className="py-2 px-3">
        <Input
          value={target}
          onChange={handleChange(setTarget)}
          className="h-9 text-sm"
      ...
      <td className="py-2 px-3">
        <Input
          value={actual}
          onChange={handleChange(setActual)}
          className="h-9 text-sm"
          placeholder="—"
      ...
      <td className="py-2 px-3 text-center">
        <div className="flex items-center justify-center gap-1">
          {dirty && (
            <Button size="icon" variant="ghost" className="h-7 w-7" onClick={handleSave} data-testid={`button-save-kpi-${item.id}`}>
              <Save className="h-3.5 w-3.5 text-green-600" />
            </Button>
          )}
          <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => onDelete(item.id)} data-testid={`button-delete-kpi-${item.id}`}>
