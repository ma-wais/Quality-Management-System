...
export default function SupportKPIs() {
  ...
  const sectionData = kpiData.filter(d => d.section.startsWith("7."));

  useEffect(() => {
    if (!isLoading && sectionData.length === 0) {
      apiRequest("POST", "/api/leadership-kpi-data/seed-section", { prefix: "7" }).then(() => {
  ...
  const section71 = sectionData.filter(d => d.section === "7.1");
  const section72 = sectionData.filter(d => d.section === "7.2");
  const section73 = sectionData.filter(d => d.section === "7.3");
  const section74 = sectionData.filter(d => d.section === "7.4");
  ...
  const handleExportImage = async () => {
    ...
    link.download = `support-kpis-${new Date().toISOString().split("T")[0]}.png`;
  ...
  const handleExportPDF = async () => {
    ...
    pdf.save(`support-kpis-${new Date().toISOString().split("T")[0]}.pdf`);
  };
        ...
        <CardContent className="space-y-6">
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b bg-muted/50">
                  <th className="text-start py-3 px-3 font-medium w-[35%]">{t('supportKpis.indicator')}</th>
                  <th className="text-start py-3 px-3 font-medium w-[30%]">{t('supportKpis.measurementMethod')}</th>
                  <th className="text-start py-3 px-3 font-medium w-[12%]">{t('supportKpis.targetLabel')}</th>
                  <th className="text-start py-3 px-3 font-medium w-[12%]">{t('supportKpis.actualValue')}</th>
            ...
            <div className="h-72 mt-4">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                  ...
                  <Tooltip
                    formatter={(value: number, name: string) => [value, name === "actual" ? t('supportKpis.actualValue') : t('supportKpis.targetLabel')]}
                    labelFormatter={(label) => { const entry = chartData.find(d => d.name === label); return entry?.fullName || label; }}
                  />
                  <Bar dataKey="target" fill="#cbd5e1" name={t('supportKpis.targetLabel')} barSize={28} radius={[4, 4, 0, 0]}>
                    <LabelList dataKey="target" position="top" fontSize={10} fill="#64748b" />
                  </Bar>
                  <Bar dataKey="actual" name={t('supportKpis.actualValue')} barSize={28} radius={[4, 4, 0, 0]}>
          ...
          {!hasActualValues && data.length > 0 && (
            <div className="text-center py-6 text-muted-foreground text-sm" data-testid={`text-no-chart-${sectionId}`}>
              {t('supportKpis.fillActualValues')}
            </div>
          )}
                    ...
                    <CardContent className="p-4">
                      ...
                      <div className="space-y-1.5">
                        <div className="flex justify-between text-xs">
                          <span className="text-muted-foreground">{t('supportKpis.targetLabel')}:</span>
                          <span className="font-medium">{item.target || "—"}</span>
                        </div>
                        <div className="flex justify-between text-xs">
                          <span className="text-muted-foreground">{t('supportKpis.actualValue')}:</span>
  ...
  if (isLoading) {
    return (
      <div className="p-6">
        <PageHeader title={t('supportKpis.title')} description={t('supportKpis.description')} clause="7" />
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={handleExportImage} data-testid="button-export-image">
            ...
            {t('supportKpis.exportImage')}
          </Button>
          <Button variant="outline" size="sm" onClick={handleExportPDF} data-testid="button-export-pdf">
            <FileDown className="h-4 w-4 ltr:mr-2 rtl:ml-2" />
            {t('supportKpis.exportPDF')}
          </Button>
        </div>
      </div>

      <div ref={kpiRef}>
        {renderKpiSection("7.1", t('supportKpis.section71Title'), section71)}
        {renderKpiSection("7.2", t('supportKpis.section72Title'), section72)}
        {renderKpiSection("7.3", t('supportKpis.section73Title'), section73)}
        {renderKpiSection("7.4", t('supportKpis.section74Title'), section74)}
      </div>
    </div>
  );
}
