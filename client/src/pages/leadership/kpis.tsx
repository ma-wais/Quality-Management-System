...
export default function LeadershipKPIs() {
  ...
  useEffect(() => {
    if (kpiData.length === 0 && !isLoading) {
      apiRequest("POST", "/api/leadership-kpi-data/seed").then(() => {
  ...
  const section51 = kpiData.filter(d => d.section === "5.1");
  const section53 = kpiData.filter(d => d.section === "5.3");
  ...
  const handleExportImage = async () => {
    ...
    link.download = `leadership-kpis-${new Date().toISOString().split("T")[0]}.png`;
  ...
  const handleExportPDF = async () => {
    ...
    pdf.save(`leadership-kpis-${new Date().toISOString().split("T")[0]}.pdf`);
  };
        ...
        <CardContent className="space-y-6">
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b bg-muted/50">
                  <th className="text-start py-3 px-3 font-medium w-[35%]">{t('leadershipKpis.indicator')}</th>
                  <th className="text-start py-3 px-3 font-medium w-[30%]">{t('leadershipKpis.measurementMethod')}</th>
                  <th className="text-start py-3 px-3 font-medium w-[12%]">{t('leadershipKpis.target')}</th>
                  <th className="text-start py-3 px-3 font-medium w-[12%]">{t('leadershipKpis.actualValue')}</th>
                ...
                <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                  ...
                  <Tooltip
                    formatter={(value: number, name: string) => [
                      value,
                      name === "actual" ? t('leadershipKpis.actualValue') : t('leadershipKpis.target')
                    ]}
                  ...
                  />
                  <Bar dataKey="target" fill="#cbd5e1" name={t('leadershipKpis.target')} barSize={28} radius={[4, 4, 0, 0]}>
                    <LabelList dataKey="target" position="top" fontSize={10} fill="#64748b" />
                  </Bar>
                  <Bar dataKey="actual" name={t('leadershipKpis.actualValue')} barSize={28} radius={[4, 4, 0, 0]}>
          ...
          {!hasActualValues && data.length > 0 && (
            <div className="text-center py-6 text-muted-foreground text-sm" data-testid={`text-no-chart-${sectionId}`}>
              {t('leadershipKpis.fillActualValues')}
            </div>
          )}
                    ...
                    <CardContent className="p-4">
                      ...
                      <div className="space-y-1.5">
                        <div className="flex justify-between text-xs">
                          <span className="text-muted-foreground">{t('leadershipKpis.target')}:</span>
                          <span className="font-medium">{item.target || "—"}</span>
                        </div>
                        <div className="flex justify-between text-xs">
                          <span className="text-muted-foreground">{t('leadershipKpis.actualValue')}:</span>
  ...
  if (isLoading) {
    return (
      <div className="p-6">
        <PageHeader title={t('leadershipKpis.title')} description={t('leadershipKpis.description')} clause="5" />
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={handleExportImage} data-testid="button-export-image">
            ...
            {t('leadershipKpis.exportImage')}
          </Button>
          <Button variant="outline" size="sm" onClick={handleExportPDF} data-testid="button-export-pdf">
            <FileDown className="h-4 w-4 ltr:mr-2 rtl:ml-2" />
            {t('leadershipKpis.exportPDF')}
          </Button>
        </div>
      </div>

      <div ref={kpiRef}>
        {renderKpiSection("5.1", t('leadershipKpis.section51Title'), section51)}
        {renderKpiSection("5.3", t('leadershipKpis.section53Title'), section53)}
      </div>
    </div>
  );
}
